...
var pluginName = 'SimpleMarquee';function Plugin (element, options) {
...
this.counter = 0;this.timeLeft = 0;
this.currentPos = 0;
this.distanceLeft = 0;
...
$(config.marquee_class).remove();if(spawnAmount<=2){
spawnAmount = 3;
} else {
spawnAmount++;
}var totalContentWidth = (contentWidth + config.padding)*spawnAmount;var endPoint = -(totalContentWidth - containerWidth);var totalDistance =  containerWidth - endPoint;
...
.mouseenter(function() {if ((plugin.documentHasFocus == true) && (plugin.marqueeHovered == false)){
plugin.marqueeHovered = true;for (var key in plugin.marqueeSpawned){
plugin.marqueeSpawned[key].el.clearQueue().stop();
plugin.marqueeSpawned[key].hovered = true;
}}})
.mouseleave(function() {if ((plugin.documentHasFocus == true) && (plugin.marqueeHovered == true)){for (var key in plugin.marqueeSpawned){
plugin.marqueeManager(plugin.marqueeSpawned[key]);
}plugin.marqueeHovered = false;
}
});} else {newElement = $('<div class="marquee-' + (i+1) + '">' + marqueeContent + '</div>') ;}plugin.marqueeSpawned[i] = new marqueeObj(newElement);$(config.container_class).append(newElement);plugin.marqueeSpawned[i].currentPos = (widthToIgnore + (contentWidth*i))+(config.padding*i);
plugin.marqueeSpawned[i].name = '.marquee-'+(i+1);plugin.marqueeSpawned[i].totalDistance = totalDistance;
plugin.marqueeSpawned[i].containerWidth = containerWidth;
plugin.marqueeSpawned[i].contentWidth = contentWidth;
plugin.marqueeSpawned[i].endPoint = endPoint;
plugin.marqueeSpawned[i].duration = config.duration;
plugin.marqueeSpawned[i].padding = config.padding;plugin.marqueeSpawned[i].el.css(config.direction, plugin.marqueeSpawned[i].currentPos+config.padding +'px');if (plugin.documentHasFocus == true){
...
var config = plugin.settings;
if (marqueed_el.hovered == false) {if (marqueed_el.counter > 0) {
marqueed_el.timeLeft = marqueed_el.duration;
marqueed_el.el.css(config.direction, marqueed_el.containerWidth +'px');
marqueed_el.currentPos = marqueed_el.containerWidth;
marqueed_el.distanceLeft = marqueed_el.totalDistance - (marqueed_el.containerWidth - plugin.getPosition(elName));} else {
...
callback: function() {
var onComplete = this.settings.onComplete;if ( typeof onComplete === 'function' ) {
...
})( jQuery, window, document );function putLocalStorageValue(key, value) {
if (localStorage) {
...
scrollTopBottom();
siteFixedAd();
cookiePolicy();$(".post-sharing-top").html(writeSocialShareLinks('blog'));
...
event.preventDefault();
socialWebShare();
});homePageSlideshow();});function searchArticlesListPage() {
...
const noResults = document.getElementById('noResults');searchInput.addEventListener('input', () => {
...
noResults.querySelector('span').textContent = query;
});
